#!/usr/bin/env python3
"""
Enhanced Microbiome PDF Report Generator - Week 1 MVP
...
class EnhancedMicrobiomeReportGenerator:
    ...
    # Polish translations (inherited from advanced generator)
    TRANSLATIONS = {
        ...
        'clinical_interpretation': 'INTERPRETACJA KLINICZNA',
    ...
    def _draw_header(self, c: canvas.Canvas, patient_info: Dict, page_num: int = 1):
        ...
        c.drawString(220, self.PAGE_HEIGHT - 50, self.TRANSLATIONS['title'])
        ...
        c.drawString(info_x, info_y, f"{self.TRANSLATIONS['sample_number']}: {patient_info.get('sample_number', 'N/A')}")
        c.drawString(info_x, info_y - 15, f"{self.TRANSLATIONS['patient_name']}: {patient_info.get('name', 'N/A')}")
        c.drawString(info_x, info_y - 30, f"{self.TRANSLATIONS['species_age']}: {patient_info.get('species', 'Koń')}, {patient_info.get('age', 'N/A')}")
        c.drawString(info_x, info_y - 45, f"{self.TRANSLATIONS['date_received']}: {patient_info.get('date_received', datetime.now().strftime('%d.%m.%Y r.'))}")
        
        # Page number
        c.setFont("Helvetica", 9)
        c.drawRightString(self.PAGE_WIDTH - 40, 30, f"Strona {page_num}")
    ...
    def _draw_footer(self, c: canvas.Canvas, patient_info: Dict):
        ...
        c.drawString(40, 45, f"{self.TRANSLATIONS['performed_by']}: {patient_info.get('performed_by', 'Laboratorium HippoVet+')}")
        c.drawRightString(self.PAGE_WIDTH - 40, 45, f"{self.TRANSLATIONS['date_analyzed']}: {datetime.now().strftime('%d.%m.%Y r.')}")
    ...
    def _create_species_chart(self, species_data: pd.DataFrame, output_path: str):
        ...
        ax.set_xlabel('Udział procentowy (%)', fontsize=12)
        ax.set_title('Rozkład gatunków bakterii', fontsize=14, weight='bold', color='#1E3A8A')
    ...
    def _create_phylum_chart(self, phylum_dist: Dict[str, float], output_path: str):
        ...
        # Customize
        ax.set_ylabel('Udział procentowy (%)', fontsize=12)
        ax.set_title(self.TRANSLATIONS['phylum_distribution'], fontsize=14, weight='bold', color='#1E3A8A')
        ...
        # Legend
        ax.text(0.02, 0.98, self.TRANSLATIONS['reference_range'] + ' (---)', 
    ...
    def _get_clinical_interpretation(self, di_score: float) -> str:
        ...
        if di_score < 20:
            return "Mikrobiota jelitowa znajduje się w równowadze. Skład bakteryjny mieści się w zakresach referencyjnych."
        elif di_score < 50:
            return "Obserwuje się umiarkowane zaburzenia składu mikrobioty jelitowej. Zalecana jest obserwacja i ewentualna modyfikacja diety."
        else:
            return "Stwierdzono znaczące zaburzenia równowagi mikrobioty jelitowej. Zalecana jest konsultacja weterynaryjna i wdrożenie odpowiedniego protokołu terapeutycznego."
    ...
    def generate_report(self, output_file: str, patient_info: Dict):
        ...
        try:
            ...
            c.setFillColor(self.BRAND_COLORS['primary'])
            c.drawString(60, 350, self.TRANSLATIONS['executive_summary'])
            ...
            c.setFillColor(self.BRAND_COLORS['text'])
            summary_text = [
                f"Przeanalizowano próbkę mikrobiomu jelitowego pacjenta {patient_info.get('name', 'N/A')}.",
                f"Zidentyfikowano {len(species_data)} gatunków bakterii.",
                f"Wskaźnik dysbiozy (DI): {di_score:.1f}/100",
                f"Dominujące phylum: {max(phylum_dist, key=phylum_dist.get)} ({phylum_dist[max(phylum_dist, key=phylum_dist.get)]:.1f}%)"
            ]
            ...
            c.setFont("Helvetica-Bold", 14)
            c.setFillColor(self.BRAND_COLORS['primary'])
            c.drawString(60, self.PAGE_HEIGHT - 160, self.TRANSLATIONS['microbiome_profile'])
            ...
            c.setFillColor(self.BRAND_COLORS['primary'])
            c.drawString(60, 350, self.TRANSLATIONS['clinical_interpretation'])
            ...
            c.setFont("Helvetica-Bold", 14)
            c.setFillColor(self.BRAND_COLORS['primary'])
            c.drawString(60, self.PAGE_HEIGHT - 160, self.TRANSLATIONS['biochemical_analysis'])
            ...
            c.setFont("Helvetica-Bold", 14)
            c.setFillColor(self.BRAND_COLORS['primary'])
            c.drawString(60, self.PAGE_HEIGHT - 160, self.TRANSLATIONS['recommendations'])
    ...
    def _draw_analysis_table(self, c: canvas.Canvas, x: float, y: float):
        ...
        # Table data
        data = [
            ['Parametr', 'Wynik', 'Zakres referencyjny', 'Status'],
            ['pH', '7.2', '6.8 - 7.4', 'Norma'],
            ['Konsystencja', 'Uformowany', 'Uformowany', 'Norma'],
            ['Krew utajona', 'Negatywny', 'Negatywny', 'Norma'],
            ['Tłuszcze', 'Śladowe', 'Brak/Śladowe', 'Norma'],
            ['Białko', 'Negatywny', 'Negatywny', 'Norma']
        ]
    ...
    def _get_recommendations(self, di_score: float) -> List[str]:
        ...
        if di_score < 20:
            return [
                "Utrzymać dotychczasową dietę i schemat żywienia",
                "Kontynuować regularne badania kontrolne co 12 miesięcy",
                "Zapewnić dostęp do świeżej wody i wysokiej jakości paszy"
            ]
        elif di_score < 50:
            return [
                "Rozważyć suplementację probiotykami przez okres 4-6 tygodni",
                "Zwiększyć udział włókna w diecie",
                "Ograniczyć stosowanie koncentratów paszowych",
                "Przeprowadzić badanie kontrolne za 3 miesiące",
                "Monitorować konsystencję kału i apetyt"
            ]
        else:
            return [
                "Pilna konsultacja z lekarzem weterynarii",
                "Wdrożyć protokół suplementacji probiotycznej",
                "Przeprowadzić pełne badanie gastroenterologiczne",
                "Rozważyć modyfikację całego programu żywieniowego",
                "Wykluczyć obecność pasożytów przewodu pokarmowego",
                "Kontrolne badanie mikrobiologiczne za 4-6 tygodni"
            ]
...
def main():
    ...
    parser.add_argument('--name', default='Patient',
                        help='Patient name')
    parser.add_argument('--species', default='Koń',
                        help='Species')
    parser.add_argument('--age', default='Unknown',
                        help='Patient age')
    parser.add_argument('--sample', default='001',
                        help='Sample number')
    parser.add_argument('--performed-by', default='Laboratorium HippoVet+',
                        help='Who performed the analysis')
    parser.add_argument('--requested-by', default='Lekarz prowadzący',
